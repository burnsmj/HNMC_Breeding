...
heterosis_plot = mid_high_data %>%
  mutate(Heterosis_Type = factor(Heterosis_Type, levels = c('MPH', 'HPH'))) %>%
...
```{r loading SNP data}
geno_data = read_delim('../Data/Hapmap_for_GP.txt', delim = '\t') %>%
  # rename(rs = `rs#`,
  #        assembly = `assembly#`) %>%
  unique() %>%
  arrange(chrom, pos)
...
ggsave('/Users/michael/Desktop/Grad_School/Research/HNMC_Breeding/Outputs/imputation_plot.png',imputation_plot, device = 'png', width = 3.75, height = 3.5, units = 'in', bg = 'transparent')
...
```{r save imputed snp file}
 #write_delim(snp_data_imputed, '~/Desktop/Grad_School/Research/HNMC_Breeding/Data/GWAS_Based_Markers.imputed.hmp.txt', delim = '\t')

# Read in imputed snp data
snp_data_imputed = read_delim('../Data/GWAS_Based_Markers.imputed.hmp.txt', delim = '\t')
```
...
write_delim(hybrid_hapmap, '~/Desktop/Grad_School/Research/HNMC_Breeding/Data/GWAS_Based_Hybrid_Hapmap.hmp.txt', delim = '\t')
```
...
hybrid_phenotype_data_2022 = hybrid_blues_2022 %>% 
...
hybrid_phenotype_data_2023 = hybrid_blues_2023 %>% 
  select(egg_parent, pollen_parent, Genotype, BLUE) %>%
  filter(!is.na(BLUE)) %>%
  filter(Genotype %in% hybrid_num$taxa,
         Genotype %in% hybrid_blues_2022$Genotype) %>%
...
hybrid_num %>%
  ...
  mutate(.before = 2,
         Pheno_Present = case_when(taxa %in% hybrid_num_sub$taxa ~ 'Yes',
                                   !taxa %in% hybrid_num_sub$taxa ~ 'No')) %>%
  write_delim('../Outputs/widiv_hybrids_gs_num.hmp.txt', delim = '\t')
```
      ...
      gp_outcomes_cv1 = gp_outcomes_cv1 %>%
        bind_rows(tibble(Fold = fold,
                         ...
                         Rs = cor(phenotype_pred,
                                  test_pheno[,'BLUE'],
                                  method = 'spearman'),
                         RMSE_M = RMSE(phenotype_pred[test_pheno$pollen_parent == 'MO17'],
                                     test_pheno[test_pheno$pollen_parent == 'MO17','BLUE'][[1]]),
                         Rs_M = cor(phenotype_pred[test_pheno$pollen_parent == 'MO17'],
                                    test_pheno[test_pheno$pollen_parent == 'MO17','BLUE'],
                                    method = 'spearman'),
                         RMSE_B = RMSE(phenotype_pred[test_pheno$pollen_parent == 'B73'],
                                     test_pheno[test_pheno$pollen_parent == 'B73','BLUE'][[1]]),
                         Rs_B = cor(phenotype_pred[test_pheno$pollen_parent == 'B73'],
                                    test_pheno[test_pheno$pollen_parent == 'B73','BLUE'],
                                    method = 'spearman')))
    }
  }
}
...
ggsave('../Outputs/gp_correlation_plot.png', prediction_correlation_plot, device = 'png', width = 3.75, height = 4.5, units = 'in')
...
gp_plot_cv1 = gp_outcomes_cv1 %>%
  pivot_longer(cols = c(RMSE, Rs, RMSE_M, Rs_M, RMSE_B, Rs_B),
               names_to = 'Metric',
               values_to = 'Performance') %>%
  mutate(Group = case_when(Metric == 'RMSE' ~ 'Overall',
                            Metric == 'Rs' ~ 'Overall',
                            Metric == 'RMSE_M' ~ 'Mo17',
                            Metric == 'Rs_M' ~ 'Mo17',
                            Metric == 'RMSE_B' ~ 'B73',
         ...
         Metric = case_when(Metric == 'RMSE' ~ 'RMSE',
                            ...
                            Metric == 'RMSE_M' ~ 'RMSE',
                            Metric == 'Rs_M' ~ 'Spearman R',
                            Metric == 'RMSE_B' ~ 'RMSE',
                            Metric == 'Rs_B' ~ 'Spearman R'),
...
ggsave('../Outputs/gp_plot_cv1.png', gp_plot_cv1, device = 'png', width = 3.75, height = 4.5, units = 'in')
...
gp_outcomes_cv1 %>%
  ...
  pivot_wider(id_cols = c(Metric, Stat),
              names_from = group,
              values_from = Value) %>%
  write_csv('../Outputs/gp_outcomes_cv1.csv')
```
